...
{
    public partial class MVBMehsul : Form
    {
        ...
        private void MVBMehsul_Load(object sender, EventArgs e)
        {

            this.Text = " Marketlərə verilən bonus məhsullar";
            WindowState = FormWindowState.Maximized;
            query = "select SciT.AD, mt.AD , mt.Nov, mt.OlcuVahidi, ((select ifnull(sum(BonusMiqdar), 0) from AlisSatisMehsulTable where OperationType=2 and BonusMehsulID =asmt.BonusMehsulID And AlisSatisMehsulTable.DeleteInfo is not 2)) as BonusMiqdar, asmt.BonusMehsulID, OperationType, SciT.SifarisciID from AlisSatisMehsulTable asmt join MehsulTable mt on asmt.BonusMehsulID=mt.MehsulID join SifarisciTable SciT on asmt.SifarisciID = SciT.SifarisciID where OperationType=2 and asmt.DeleteInfo is not 2 group by SciT.AD, mt.AD , mt.Nov, mt.OlcuVahidi";
