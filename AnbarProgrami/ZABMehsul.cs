...
{
    public partial class ZABMehsul : Form
    {
        ...
        private void ZABMehsul_Load(object sender, EventArgs e)
        {
            this.Text = "Zavodlardan alınan bonus məhsullar";

            WindowState = FormWindowState.Maximized;
            query = "select SciT.AD, mt.AD , mt.Nov, mt.OlcuVahidi, ((select ifnull(sum(BonusMiqdar), 0) from AlisSatisMehsulTable where OperationType=0 and BonusMehsulID =asmt.BonusMehsulID And AlisSatisMehsulTable.DeleteInfo is not 2)) as BonusMiqdar, asmt.BonusMehsulID, OperationType, SciT.SifarisciID from AlisSatisMehsulTable asmt join MehsulTable mt on asmt.BonusMehsulID=mt.MehsulID join SifarisciTable SciT on asmt.SifarisciID = SciT.SifarisciID where OperationType=0 and asmt.DeleteInfo is not 2 group by SciT.AD, mt.AD , mt.Nov, mt.OlcuVahidi";
