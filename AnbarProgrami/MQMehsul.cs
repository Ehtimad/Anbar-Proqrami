...
{
    public partial class MQMehsul : Form
    {
        ...
        private void MQMehsul_Load(object sender, EventArgs e)
        {
            this.Text = " Marketlərdən qayıdan məhsullar";
            WindowState = FormWindowState.Maximized;
            query = "select SciT.AD, mt.AD , mt.Nov, mt.OlcuVahidi, ((select ifnull(sum(Miqdari), 0) from AlisSatisMehsulTable where OperationType=3 and MehsulID =asmt.MehsulID And AlisSatisMehsulTable.DeleteInfo is not 2)) as Miqdar, ((select ifnull(sum(CemGuzestsizQiymet), 0) from AlisSatisMehsulTable where OperationType=3 and MehsulID =asmt.MehsulID And AlisSatisMehsulTable.DeleteInfo is not 2)) as CemGuzestsizQiymet, ((select ifnull(sum(CemGuzestliQiymet), 0) from AlisSatisMehsulTable where OperationType=3 and MehsulID =asmt.MehsulID And AlisSatisMehsulTable.DeleteInfo is not 2)) as CemGuzestliQiymet, asmt.MehsulID, OperationType, SciT.SifarisciID from AlisSatisMehsulTable asmt  join MehsulTable mt on asmt.MehsulID=mt.MehsulID  join SifarisciTable SciT on asmt.SifarisciID = SciT.SifarisciID where asmt.OperationType=3 And asmt.DeleteInfo is not 2 group by SciT.AD, mt.AD , mt.Nov, mt.OlcuVahidi";
