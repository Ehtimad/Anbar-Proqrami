    ...
    public partial class salesForm : Form
    {
        ...
        public void listelecomboboxfactory()
        {
            try
            {
                ...
                baqlanti.Open();
                string query = "select * from SifarisciTable Where SifarisciType=1 And DeleteInfo is not 2 order by Ad asc";
                SQLiteCommand kommand = new SQLiteCommand(query, baqlanti);
                DataTable dt = new DataTable();
                SQLiteDataAdapter da = new SQLiteDataAdapter(kommand);
                da.Fill(dt);
        ...
        public void listelecomboboxProduct()
        {
        ...
        public void listelecomboboxProductBonus()
        {
            bonusMehsulcomboBox.Items.Clear();
            bonusMehsulcomboBox.Items.Add("Yoxdur");
            SQLiteCommand kommandkat = new SQLiteCommand();
            baqlanti.Open();
            string querykat = "select * from MehsulTable Where DeleteInfo is not 2";
            kommandkat = new SQLiteCommand(querykat, baqlanti);
            DataTable dtkat = new DataTable();
            SQLiteDataAdapter dakat = new SQLiteDataAdapter(kommandkat);
            dakat.Fill(dtkat);
        ...
        public void selectlistele()
        {
            ...
            try
            {
                ...
                SQLiteCommand kommand = new SQLiteCommand("Select QaimeTable.QaimeNomresi, QaimeTable.Tarix, SciT.AD, SciT.Temsilci, SciT.WeherUnvan, SciT.RayonUnvan, SciT.QesebeUnvan, IT.Ad, mt.AD, mt.Nov, mt.OlcuVahidi, CAST ( CASE WHEN ASMT.OperationType = 0 THEN 'Alış' WHEN ASMT.OperationType = 1 THEN 'Qaytarma' WHEN ASMT.OperationType = 2 THEN 'Satış' ELSE 'Qaytarma' END AS nvarchar(55)) as OperationType, Miqdari, Qiymet, Guzest, mt1.AD, mt1.Nov, BonusMiqdar, UmumiSay, BirGuzestMebleqi, CemGuzestMebleqi, CemGuzestsizQiymet, BirGuzestliQiymet, CemGuzestliQiymet, ASMT.OperationDate, ASMT.Qeyd, CAST ( CASE WHEN ASMT.EditInfo = 1 THEN 'Redaktə olundu' END AS nvarchar(55)) as EditInfo, CAST ( CASE WHEN ASMT.DeleteInfo = 2 THEN 'Silindi' END AS nvarchar(55)) as DeleteInfo, ASMT.AlisSatisMehsulID, QaimeTable.QaimeID, mt.MehsulID, SciT.SifarisciID, BonusMehsulID from AlisSatisMehsulTable ASMT left join MehsulTable mt on mt.MehsulID=ASMT.MehsulID left join MehsulTable mt1 on mt1.MehsulID=ASMT.BonusMehsulID left join QaimeTable on ASMT.QaimeID = QaimeTable.QaimeID join SifarisciTable SciT on ASMT.SifarisciID = SciT.SifarisciID left join KategoriyaTable KT on SciT.KategoriyID=KT.KategoriyID join IsciTable IT on QaimeTable.IsciID=IT.IsciID Where " + editedsql + " ASMT.DeleteInfo " + silinensql + " And QaimeTable.Tarix >=  @Date1 And QaimeTable.Tarix <= @Date2 And OdemeType is null And (ASMT.OperationType=2 or ASMT.OperationType=3) order by AlisSatisMehsulID desc", baqlanti);
        ...
        private void SalesForm_Load(object sender, EventArgs e)
        {
            ...
            DGW.Columns[8].Caption = "Məhsulun adı";
            DGW.Columns[9].Caption = "Məhsulun növü";
            DGW.Columns[10].Caption = "Ölçü vahidi";
            DGW.Columns[11].Caption = "Əməliyyatın növü";
            DGW.Columns[12].Caption = "Məhsulun miqdarı";
            DGW.Columns[13].Caption = "Qiyməti";
            DGW.Columns[14].Caption = "Güzəşt faizi";
            DGW.Columns[15].Caption = "Hədiyyə məhsulun adı";
            DGW.Columns[16].Caption = "Hədiyyə məhsulun növü";
            DGW.Columns[17].Caption = "Hədiyyə məhsulun miqdarı";
            DGW.Columns[18].Caption = "Ümumi sayı";
            DGW.Columns[19].Caption = "Bir ədədin güzəşti";
            DGW.Columns[20].Caption = "Cəm ədədin güzəşti";
            DGW.Columns[21].Caption = "Cəm güzəştsiz qiymət";
            DGW.Columns[22].Caption = "Birinin güzəştli qiyməti";
            DGW.Columns[23].Caption = "Cəm güzəştli qiymət";
            DGW.Columns[24].Caption = "Qeyd tarixi";
            DGW.Columns[25].Caption = "Qeyd";
            DGW.Columns[26].Caption = "Redaktə olunan";
            DGW.Columns[27].Caption = "Silinən";
            DGW.Columns[28].Caption = "AlisSatisMehsulID";
            DGW.Columns[29].Caption = "QaimeID";
            DGW.Columns[30].Caption = "MehsulID";
            DGW.Columns[31].Caption = "SifarisciID";
            DGW.Columns[32].Caption = "BonusMehsulID";
        ...
        private void addBtn_Click(object sender, EventArgs e)
        {
            ...
            listelecomboboxProduct();
            listelecomboboxProductBonus();
        ...
        private void editBtn_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                productNameComboBox.Text = DGW.GetDataRow(DGW.FocusedRowHandle)[8].ToString();
                operationTypeComboBox.Text = DGW.GetDataRow(DGW.FocusedRowHandle)[11].ToString();
                productMiqdarTxt.Text = DGW.GetDataRow(DGW.FocusedRowHandle)[12].ToString();
                mehsulPriceTxt.Text = DGW.GetDataRow(DGW.FocusedRowHandle)[13].ToString();
                guzestTxt.Text = DGW.GetDataRow(DGW.FocusedRowHandle)[14].ToString();
                string bonusmad = DGW.GetDataRow(DGW.FocusedRowHandle)[15].ToString();
                ...
                {
                    bonusMehsulcomboBox.Text = DGW.GetDataRow(DGW.FocusedRowHandle)[15].ToString();
                }
                bonusMiqdarTxt.Text = DGW.GetDataRow(DGW.FocusedRowHandle)[17].ToString();
                noteTxt.Text = DGW.GetDataRow(DGW.FocusedRowHandle)[25].ToString();
        ...
        private void deleteBtn_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    int AlisSatismehsulid = Convert.ToInt32(DGW.GetDataRow(DGW.FocusedRowHandle)[28].ToString());
            ...
            try
            {
                ...
                else
                {
                    try
                    {
                        ...
                        var alissatisidqaime = (qaimeAlisSatisComboBox.SelectedItem as ComboboxItem).Value;
                        int alissatismehsulid = int.Parse(DGW.GetDataRow(DGW.FocusedRowHandle)[28].ToString());
                        ...
                        kommand.Connection = baqlanti;
                        baqlanti.Open();
                        kommand.CommandText = "Update AlisSatisMehsulTable Set QaimeID=@QaimeID, OperationType=@OperationType, SifarisciID= @SifarisciID, MehsulID=@MehsulID,  Miqdari=@Miqdari, Qiymet=@Qiymet, Guzest=@Guzest, BonusMehsulID=@BonusMehsulID, BonusMiqdar = @BonusMiqdar, UmumiSay=@UmumiSay, BirGuzestMebleqi=@BirGuzestMebleqi, CemGuzestMebleqi=@CemGuzestMebleqi, CemGuzestsizQiymet=@CemGuzestsizQiymet, BirGuzestliQiymet=@BirGuzestliQiymet, CemGuzestliQiymet=@CemGuzestliQiymet, OperationDate=@OperationDate, Qeyd=@Qeyd, EditInfo=@EditInfo Where AlisSatisMehsulID=@AlisSatisMehsulID";
                        kommand.Parameters.AddWithValue("@AlisSatisMehsulID", alissatismehsulid);
                        kommand.Parameters.AddWithValue("@QaimeID", alissatisidqaime);
        ...
        private void addGBtn_Click(object sender, EventArgs e)
        {
            try
            {
                if (factorymarketComboBox.Text.Trim() == "" || qaimeAlisSatisComboBox.Text.Trim() == "" || productNameComboBox.Text.Trim() == "" || operationTypeComboBox.Text.Trim() == "" || productMiqdarTxt.Text.Trim() == "" || mehsulPriceTxt.Text.Trim() == "")
                {
                    MessageBox.Show("Boş xana buraxmayın!");
                }
                else
                {
                    ...
                    kommand.Parameters.AddWithValue("@QaimeID", alissatisidqaime);
                    kommand.Parameters.AddWithValue("@OperationType", Convert.ToInt32(operationtype));
                    kommand.Parameters.AddWithValue("@SifarisciID", zavodmarketid);
        ...
        private DataTable ReportListborc()
        {
            ...
            baqlanti.Open();
            SQLiteCommand kommand = new SQLiteCommand("Select AST.QaimeNomresi, AST.Tarix, SciT.AD as SifrisciAd, SciT.Temsilci, SciT.WeherUnvan, SciT.RayonUnvan, SciT.QesebeUnvan, SciT.Phone, IT.Ad as IsciAd, mt.Ad as MAd, mt.Nov as MNov, mt.OlcuVahidi as MOVahidi,  CAST ( CASE WHEN ASMT.OperationType = 0 THEN 'Alış' WHEN ASMT.OperationType = 1 THEN 'Geri qaytarma'  WHEN ASMT.OperationType = 2 THEN 'Satış' WHEN ASMT.OperationType = 3 THEN 'Geri qaytarma' END AS nvarchar(55)) as OperationType,  Miqdari, Qiymet, Guzest, mt1.Ad as BMAd, mt1.Nov as BMNov, mt1.OlcuVahidi as BMOVahidi, BonusMehsulID, BonusMiqdar, ((select ifnull(sum(Miqdari), 0) from AlisSatisMehsulTable where OperationType=2 and QaimeID =ASMT.QaimeID  And AlisSatisMehsulTable.DeleteInfo is not 2) - (select ifnull(sum(Miqdari), 0) from AlisSatisMehsulTable where OperationType=3 and QaimeID =ASMT.QaimeID And AlisSatisMehsulTable.DeleteInfo is not 2))as MiqdarCemQaime,  ((select ifnull(sum(BonusMiqdar), 0) from AlisSatisMehsulTable where OperationType=2 and QaimeID =ASMT.QaimeID  And AlisSatisMehsulTable.DeleteInfo is not 2) - (select ifnull(sum(BonusMiqdar), 0) from AlisSatisMehsulTable where OperationType=3 and QaimeID =ASMT.QaimeID And AlisSatisMehsulTable.DeleteInfo is not 2))as BonusMiqdarCemQaime, UmumiSay,  ((select ifnull(sum(UmumiSay), 0) from AlisSatisMehsulTable where OperationType=2 and QaimeID =ASMT.QaimeID  And AlisSatisMehsulTable.DeleteInfo is not 2 ) - (select ifnull(sum(UmumiSay), 0) from AlisSatisMehsulTable where OperationType=3 and QaimeID =ASMT.QaimeID And AlisSatisMehsulTable.DeleteInfo is not 2 ))as UmumiSayCemQaime, BirGuzestMebleqi, CemGuzestMebleqi, BirGuzestMebleqi, CemGuzestMebleqi,  ((select ifnull(sum(CemGuzestMebleqi), 0) from AlisSatisMehsulTable where OperationType=2 and QaimeID =ASMT.QaimeID  And AlisSatisMehsulTable.DeleteInfo is not 2 ) - (select ifnull(sum(CemGuzestMebleqi), 0) from AlisSatisMehsulTable where OperationType=3 and QaimeID =ASMT.QaimeID And AlisSatisMehsulTable.DeleteInfo is not 2 ))as CemGuzestMebleqiQaime,  CemGuzestsizQiymet,  ((select ifnull(sum(CemGuzestsizQiymet), 0) from AlisSatisMehsulTable where OperationType=2 and QaimeID =ASMT.QaimeID  And AlisSatisMehsulTable.DeleteInfo is not 2 ) - (select ifnull(sum(CemGuzestsizQiymet), 0) from AlisSatisMehsulTable where OperationType=3 and QaimeID =ASMT.QaimeID And AlisSatisMehsulTable.DeleteInfo is not 2 ))as CemGuzestsizQiymetQaime,  BirGuzestliQiymet, CemGuzestliQiymet,  ((select ifnull(sum(CemGuzestliQiymet), 0) from AlisSatisMehsulTable where OperationType=2 and QaimeID =ASMT.QaimeID  And AlisSatisMehsulTable.DeleteInfo is not 2 ) - (select ifnull(sum(CemGuzestliQiymet), 0) from AlisSatisMehsulTable where OperationType=3 and QaimeID =ASMT.QaimeID And AlisSatisMehsulTable.DeleteInfo is not 2 ))as CemGuzestliQiymetQaime,  ((select ifnull(sum(CemGuzestsizQiymet), 0) from AlisSatisMehsulTable where OperationType=2 and SifarisciID =SciT.SifarisciID And AlisSatisMehsulTable.DeleteInfo is not 2 ) - (select ifnull(sum(CemGuzestsizQiymet), 0) from AlisSatisMehsulTable where OperationType=3 and SifarisciID =SciT.SifarisciID And AlisSatisMehsulTable.DeleteInfo is not 2 ))as CemGuzestsizQiymetSifarisci,  ((select ifnull(sum(CemGuzestliQiymet), 0) from AlisSatisMehsulTable where OperationType=2 and SifarisciID =SciT.SifarisciID And AlisSatisMehsulTable.DeleteInfo is not 2 ) + (select ifnull(sum(CemGuzestliQiymet), 0) from AlisSatisMehsulTable where OperationType=3 and SifarisciID =SciT.SifarisciID And AlisSatisMehsulTable.DeleteInfo is not 2 ))as CemGuzestliQiymetSifarisci,  ((select ifnull(sum(AlinanVerilenPul), 0) from AlisSatisMehsulTable where AlisSatisMehsulTable.SifarisciID =SciT.SifarisciID And AlisSatisMehsulTable.DeleteInfo is not 2))as CemOdemeSifarisci,  ((select ifnull(sum(AlinanVerilenPul), 0) from AlisSatisMehsulTable where AlisSatisMehsulTable.SifarisciID =SciT.SifarisciID And AlisSatisMehsulTable.DeleteInfo is not 2) - (select ifnull(sum(CemGuzestliQiymet), 0) from AlisSatisMehsulTable where OperationType=2 and SifarisciID =SciT.SifarisciID  And AlisSatisMehsulTable.DeleteInfo is not 2 ) + (select ifnull(sum(CemGuzestliQiymet), 0) from AlisSatisMehsulTable where OperationType=3 and SifarisciID =SciT.SifarisciID And AlisSatisMehsulTable.DeleteInfo is not 2 )) as CemBorcSifarisci,  ((select ifnull(sum(AlinanVerilenPul), 0) from AlisSatisMehsulTable where AlisSatisMehsulTable.SifarisciID =SciT.SifarisciID And AlisSatisMehsulTable.DeleteInfo is not 2) - (select ifnull(sum(CemGuzestliQiymet), 0) from AlisSatisMehsulTable where OperationType=2 and SifarisciID =SciT.SifarisciID  And AlisSatisMehsulTable.DeleteInfo is not 2 ) + (select ifnull(sum(CemGuzestliQiymet), 0) from AlisSatisMehsulTable where OperationType=3 and SifarisciID =SciT.SifarisciID And AlisSatisMehsulTable.DeleteInfo is not 2 )+ (select ifnull(sum(CemGuzestliQiymet), 0) from AlisSatisMehsulTable where OperationType=2 and QaimeID =ASMT.QaimeID  And AlisSatisMehsulTable.DeleteInfo is not 2 ) - (select ifnull(sum(CemGuzestliQiymet), 0) from AlisSatisMehsulTable where OperationType=3 and QaimeID =ASMT.QaimeID And AlisSatisMehsulTable.DeleteInfo is not 2 )) as CemBorcQaimedenEvvel,  ASMT.AlisSatisMehsulID, AST.QaimeID, ASMT.OperationType, mt.MehsulID, SciT.SifarisciID  from AlisSatisMehsulTable ASMT  left join MehsulTable mt on mt.MehsulID=asmt.MehsulID  left join MehsulTable mt1 on mt1.MehsulID=asmt.BonusMehsulID  left join QaimeTable AST on ASMT.QaimeID = AST.QaimeID  join SifarisciTable SciT on ASMT.SifarisciID = SciT.SifarisciID  left join KategoriyaTable KT on SciT.KategoriyID=KT.KategoriyID  join IsciTable IT on AST.IsciID=IT.IsciID  where Qaimenomresi=@QaimeSoz And ASMT.DeleteInfo is not 2  order by mt.Ad Asc ", baqlanti);
